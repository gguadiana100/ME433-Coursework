...
#define LOWTIME 15 // number of 48MHz cycles to be low for 0.35uS - 15
#define HIGHTIME 65 // number of 48MHz cycles to be high for 1.65uS - 65

int loopThroughColor(unsigned char a, int counter, volatile unsigned int * array);
...
void ws2812b_setup() {
    ...
    // initialize output pin B8 as off
    TRISBbits.TRISB8 = 0; // Set B8 pin to be an output 
    LATBbits.LATB8 = 0; // Set B8 output value to be off
}
...
//        nB = loopThroughColor(c[i].r, nB, delay_times); // red
//        nB = loopThroughColor(c[i].g, nB, delay_times); // green
//        nB = loopThroughColor(c[i].b, nB, delay_times); // blue
        // loop through each color bit, MSB first
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].r >> j) & 0b1) {
    ...
    for (i = 1; i < numBits; i++) {
        ...
        //while (_CP0_GET_COUNT() < delay_times[i]) {         
        }
        LATBINV = 0b100000000; // invert B8
        //LATAINV = 0b10000;
//        if (PORTBbits.RB8 == 1) {
//            LATBbits.LATB8 = 0;  
//        }
...
int loopThroughColor(unsigned char a, int counter, volatile unsigned int * array) {
    int nB = counter; 
    volatile unsigned int * delay_times = array;
    // loop through each color bit, MSB first
    for (int j = 7; j >= 0; j--) {
        // if the bit is a 1
        if ((a >> j) & 1) { /* identify the bit in c[].r, is it 1 */
            // the high is longer
            delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
            nB++;
            delay_times[nB] = delay_times[nB - 1] + LOWTIME;
